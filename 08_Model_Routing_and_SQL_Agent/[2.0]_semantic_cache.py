...
from lm_studio_backend import EMBEDDING_MODEL, create_embedding_model
...
class SemanticCache:
    def __init__(self, threshold: float = CACHE_THRESHOLD):
        ...
        try:
            self.embeddings = create_embedding_model(EMBEDDING_MODEL)
    ...
    def get_cached_response(self, query: str) -> Optional[str]:
        ...
        if score < self.threshold:
            print(f"      [OK] CACHE HIT! (Saved API Call)")
            return cached_response
        
        print(f"      [ERROR] CACHE MISS (Distance {score:.4f} > {self.threshold})")
        return None
